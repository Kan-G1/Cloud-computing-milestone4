...
# TODO: Set the project ID for your GCP project
project_id = "myride-478722"
topic_name = "election"
...
messageReceived = False    # A flag to block the machine from sending a new vote unless the current vote is processed.
...
# the condition used for filtering the messages to be received
...
# the main thread
while True:
    # randomly generate a new vote (to mimic a real voting machine)
    last_uuid = str(uuid.uuid1())
    value = {
        'machine_ID': machineID,
        'voter_ID': int(random.random() * 100),
        'voting': int(random.random() * 5),
        'election_ID': electionID,
        'UUID': last_uuid,
        'timestamp': int(1000 * time.time())
    }

    messageReceived = False  # A flag to block the main thread until result is received
