...
def callback(message: pubsub_v1.subscriber.message.Message) -> None:
    ...
    if Redis.exists(redis_key):
        ...
        future = publisher.publish(
            ...
            function="result",
            machineID=str(message_data['machine_ID'])
        )
    else:
        ...
        value = {
            ...
            'voting': message_data['voting'],
            'election_ID': message_data['election_ID'],
    ...
    # Report to Google Pub/Sub the successful processing of the received messages
    message.ack()

# create a subscriber to the subscriber for the project using the subscription_id
subscriber = pubsub_v1.SubscriberClient()
subscription_path = subscriber.subscription_path(project_id, subscription_id)
sub_filter = "attributes.function=\"submit vote\""  # the condition used for filtering the messages to be received
...
with subscriber:
    ...
    # The callback function will be called for each message matching the filter from the topic.
